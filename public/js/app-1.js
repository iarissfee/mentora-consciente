...
function bindGlobal(){
  ...
  $('#admin-button').onclick=()=>location.hash='#admin';
...
async function handleLogin(ev){ev.preventDefault();const fd=new FormData(ev.currentTarget);try{$('#auth-message').textContent='';await api('/api/auth/login',{method:'POST',body:{email:fd.get('email'),password:fd.get('password')}});await refreshMe();$('#auth-dialog').close();toast('Bienvenida al campus');renderRoute();if(state.pendingCheckout){const p=state.pendingCheckout;state.pendingCheckout=null;setTimeout(()=>openCheckout(p.type,p.item),200)}}catch(x){$('#auth-message').textContent=x.message}}
async function handleRegister(ev){ev.preventDefault();const fd=new FormData(ev.currentTarget);try{$('#auth-message').textContent='';await api('/api/auth/register',{method:'POST',body:{name:fd.get('name'),email:fd.get('email'),password:fd.get('password')}});await refreshMe();$('#auth-dialog').close();toast('Cuenta creada');renderRoute();if(state.pendingCheckout){const p=state.pendingCheckout;state.pendingCheckout=null;setTimeout(()=>openCheckout(p.type,p.item),200)}}catch(x){$('#auth-message').textContent=x.message}}

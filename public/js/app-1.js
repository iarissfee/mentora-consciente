...
function bindGlobal(){
  ...
  $('#campus-link').addEventListener('click',ev=>{if(location.hash==='#campus'){ev.preventDefault();renderRoute()}});
...
async function goAfterAuth(hash){if(location.hash===hash)await renderRoute();else location.hash=hash}
async function handleLogin(ev){ev.preventDefault();const fd=new FormData(ev.currentTarget);try{$('#auth-message').textContent='';await api('/api/auth/login',{method:'POST',body:{email:fd.get('email'),password:fd.get('password')}});await refreshMe();$('#auth-dialog').close();toast(state.me?.user?.role==='admin'?'Panel de administración listo':'Bienvenida al campus');if(state.pendingCheckout&&state.me?.user?.role!=='admin'){const p=state.pendingCheckout;state.pendingCheckout=null;await renderRoute();setTimeout(()=>{if(!state.pendingCheckout)openCheckout(p.type,p.item)},200);return}if(state.me?.user?.role==='admin'){state.pendingCheckout=null;await goAfterAuth('#admin/dashboard')}else{await goAfterAuth('#campus')}}catch(x){$('#auth-message').textContent=x.message}}
async function handleRegister(ev){ev.preventDefault();const fd=new FormData(ev.currentTarget);try{$('#auth-message').textContent='';await api('/api/auth/register',{method:'POST',body:{name:fd.get('name'),email:fd.get('email'),password:fd.get('password')}});await refreshMe();$('#auth-dialog').close();toast('Cuenta creada');await goAfterAuth('#campus');if(state.pendingCheckout){const p=state.pendingCheckout;state.pendingCheckout=null;setTimeout(()=>openCheckout(p.type,p.item),200)}}catch(x){$('#auth-message').textContent=x.message}}

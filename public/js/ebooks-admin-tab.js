...
(function(){
  ...
  style.textContent=`
    ...
    .mc-ebooks-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.mc-ebook-card-admin{background:#fff;border:2px solid #ddd8d2;padding:20px}.mc-ebook-card-top{display:grid;grid-template-columns:110px 1fr;gap:16px;align-items:start}.mc-ebook-card-top img{width:110px;aspect-ratio:3/4;object-fit:cover;background:#eee9e2}.mc-ebook-card-admin h3{font-family:"Bodoni Moda",Georgia,serif;font-size:28px;margin:0 0 4px;color:#173a27}.mc-ebook-status{display:inline-block;font-size:11px;letter-spacing:.08em;text-transform:uppercase;padding:5px 8px;background:#edf4ee;color:#244b32;margin-bottom:8px}.mc-ebook-status.missing{background:#f5ece7;color:#8a4b38}.mc-ebook-admin-form{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:18px}.mc-ebook-admin-form label{display:flex;flex-direction:column;gap:6px;font-size:12px}.mc-ebook-admin-form label.wide{grid-column:1/-1}.mc-ebook-admin-form input,.mc-ebook-admin-form select{width:100%;padding:11px;border:1px solid #d7d2cc;background:#fff}.mc-ebook-cover-box,.mc-ebook-pdf-box{margin-top:16px;padding:15px;background:#f5f1ec;border:1px solid #e2ddd7}.mc-ebook-cover-box strong,.mc-ebook-pdf-box strong{display:block;margin-bottom:5px}.mc-ebook-cover-edit{display:grid;grid-template-columns:110px 1fr;gap:14px;align-items:start}.mc-ebook-cover-edit img{width:110px;aspect-ratio:3/4;object-fit:cover;background:#e9e4dd;border:1px solid #d9d2ca}.mc-ebook-cover-actions,.mc-ebook-pdf-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.mc-ebook-cover-actions button,.mc-ebook-pdf-actions button{padding:10px 13px;border:1px solid #173a27;background:#173a27;color:#fff;cursor:pointer}.mc-ebook-cover-actions .danger,.mc-ebook-pdf-actions .danger{background:#fff;color:#8b3d35;border-color:#b58c86}.mc-ebook-save{grid-column:1/-1;background:#173a27;color:#fff;border:0;padding:12px;font-weight:700;cursor:pointer}.mc-ebook-file{margin-top:10px;width:100%}.mc-cover-help{display:block;margin-top:7px;color:#746f69;font-size:11px;line-height:1.4}
    @media(max-width:820px){.mc-ebooks-preview-grid,.mc-ebooks-grid{grid-template-columns:1fr}.mc-ebook-admin-form{grid-template-columns:1fr}.mc-ebook-admin-form label.wide,.mc-ebook-save{grid-column:auto}.mc-ebooks-preview-head{align-items:flex-start;flex-direction:column}.mc-ebook-cover-edit{grid-template-columns:90px 1fr}.mc-ebook-cover-edit img{width:90px}}
  `;
  ...
  function coverOf(x){return x.display_cover_url||x.cover_url||''}
  ...
  function bindEbooks(root){
    root.querySelectorAll('[data-ebook-id]').forEach(card=>{
      ...
      card.querySelector('[data-upload-cover]').onclick=async()=>{if(!coverFile.files?.[0])return toast('Elegí primero la imagen de portada.');const fd=new FormData();fd.append('cover',coverFile.files[0]);try{await api(`/api/admin/ebooks/${id}/cover`,{method:'POST',body:fd});toast('Portada actualizada en todos lados');await loadAdminTab()}catch(x){toast(x.message)}};
      const delCover=card.querySelector('[data-delete-cover]');if(delCover)delCover.onclick=async()=>{if(!confirm('¿Eliminar la portada que subiste?'))return;try{await api(`/api/admin/ebooks/${id}/cover`,{method:'DELETE'});toast('Portada subida eliminada');await loadAdminTab()}catch(x){toast(x.message)}};
      card.querySelector('[data-upload]').onclick=async()=>{if(!pdfFile.files?.[0])return toast('Elegí primero el PDF.');const fd=new FormData();fd.append('pdf',pdfFile.files[0]);try{await api(`/api/admin/ebooks/${id}/pdf`,{method:'POST',body:fd});toast('PDF guardado');await loadAdminTab()}catch(x){toast(x.message)}};

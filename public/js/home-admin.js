...
(function(){
  ...
  function homeAdminHtml(d){const s=d.settings||{},programs=d.programs||[];return `
    ...
    <section class="panel"><div class="mc-panel-heading"><div><p class="mc-kicker">NUEVA TARJETA</p><h3>Agregar programa o mentoría</h3><p>Estas son las tarjetas que aparecen en la Home pública.</p></div></div>
      <form id="new-home-program" class="form-grid">
        ...
        <label style="grid-column:1/-1">Descripción<textarea name="description" rows="3" maxlength="700"></textarea></label>
        <label>Texto del botón<input name="button_label" value="Ver programa" maxlength="80"></label><label>Estilo<select name="style"><option value="standard">Normal</option><option value="popular">Popular</option><option value="club">Club / oscuro</option></select></label>
        <label style="grid-column:1/-1">Destino del botón<input name="target_url" value="/campus.html#courses" placeholder="/campus.html#courses"></label>
  ...
  function homeProgramEditor(p){return `<details class="mc-manage-course" open><summary><div><span class="mc-course-state ${p.active?'live':'draft'}">${p.active?'VISIBLE':'OCULTA'}</span><h3>${e(p.title)}</h3><p>${fmtMoney(p.price,p.currency)} · orden ${Number(p.sort_order||0)}</p></div><span class="mc-open-label">Gestionar ▾</span></summary><div class="mc-manage-body"><form class="form-grid home-program-form" data-home-program="${p.id}"><label>Título<input name="title" value="${e(p.title)}" required></label><label>Precio<input name="price" type="number" min="0" step="1" value="${Number(p.price||0)}"></label><label>Moneda<select name="currency"><option ${p.currency==='EUR'?'selected':''}>EUR</option><option ${p.currency==='USD'?'selected':''}>USD</option></select></label><label>Orden<input name="sort_order" type="number" value="${Number(p.sort_order||0)}"></label><label style="grid-column:1/-1">Descripción<textarea name="description" rows="3">${e(p.description||'')}</textarea></label><label>Texto del botón<input name="button_label" value="${e(p.button_label||'Ver programa')}"></label><label>Estilo<select name="style"><option value="standard" ${p.style==='standard'?'selected':''}>Normal</option><option value="popular" ${p.style==='popular'?'selected':''}>Popular</option><option value="club" ${p.style==='club'?'selected':''}>Club / oscuro</option></select></label><label style="grid-column:1/-1">Destino del botón<input name="target_url" value="${e(p.target_url||'/campus.html#courses')}"></label><label>Etiqueta<input name="badge" value="${e(p.badge||'')}"></label><label><span><input type="checkbox" name="active" ${p.active?'checked':''}> Visible en Home</span></label><button class="btn btn-primary" type="submit">Guardar tarjeta</button><button class="mini-btn mc-delete" type="button" data-delete-home-program="${p.id}">Eliminar tarjeta</button></form></div></details>`}
  ...
  function formProgram(f){return{title:f.get('title'),description:f.get('description'),price:Number(f.get('price')),currency:f.get('currency'),button_label:f.get('button_label'),target_url:f.get('target_url'),badge:f.get('badge'),style:f.get('style'),active:!!f.get('active'),sort_order:Number(f.get('sort_order'))}}
})();
